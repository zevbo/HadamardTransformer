...
#define cudaHostAllocPortable                                                  \
  1                           ///< Pinned memory accessible by all CUDA contexts
...
#define cudaDeviceLmemResizeToMax                                              \
  16 ///< Device flag - Keep local memory allocation after launch
...
enum cudaFuncCache {
  cudaFuncCachePreferNone =
      0, ///< Default function cache configuration, no preference
  cudaFuncCachePreferShared =
      1, ///< Prefer larger shared memory and smaller L1 cache
  cudaFuncCachePreferL1 =
      2 ///< Prefer larger L1 cache and smaller shared memory
};
...
enum cudaDeviceAttr {
  ...
  cudaDevAttrGpuOverlap = 15, /**< Device can possibly copy memory and execute a
                                 kernel concurrently */
 ...
 *                   policy  (I) Parameter influencing the selection process in
 * case no fully matching cubin can be found, but instead a choice can be made
 * between ptx compilation or selection of a cubin for a less capable GPU.
...
void fatGetCubinForGpuWithPolicy(__cudaFatCudaBinary *binary,
                                 ...
                                 char *gpuName, char **cubin,
                                 char **dbgInfoFile);

#define fatGetCubinForGpu(binary, gpuName, cubin, dbgInfoFile)                 \
  fatGetCubinForGpuWithPolicy(binary, __cudaFatAvoidPTX, gpuName, cubin,       \
                              dbgInfoFile)

/*
 * Function        : Check if a binary will be JITed for the specified target
 * architecture Parameters      : binary  (I) Fat binary policy  (I) Compilation
 * policy, as described by fatGetCubinForGpuWithPolicy gpuName (I) Name of
 * target GPU ptx     (O) PTX string to be JITed Function Result : True if the
 * given binary will be JITed; otherwise, False
 */
unsigned char fatCheckJitForGpuWithPolicy(__cudaFatCudaBinary *binary,
                                          __cudaFatCompilationPolicy policy,
                                          char *gpuName, char **ptx);

#define fatCheckJitForGpu(binary, gpuName, ptx)                                \
  fatCheckJitForGpuWithPolicy(binary, __cudaFatAvoidPTX, gpuName, ptx)

/*
 * Function        : Free information previously obtained via function
 * fatGetCubinForGpu. Parameters      : cubin   (I) Cubin text string to free
...
extern cudaError_t cudaMalloc3DArray(struct cudaArray **arrayPtr,
                                     ...
                                     struct cudaExtent extent);
extern cudaError_t cudaMemset3D(struct cudaPitchedPtr pitchedDevPtr, int value,
                                struct cudaExtent extent);
...
extern cudaError_t cudaHostAlloc(void **pHost, size_t bytes,
                                 unsigned int flags);
...
extern cudaError_t cudaHostRegister(void *pHost, size_t bytes,
                                    unsigned int flags);
...
extern cudaError_t cudaMemcpyFromArray(void *dst, const struct cudaArray *src,
                                       ...
                                       size_t count, enum cudaMemcpyKind kind);
extern cudaError_t
cudaMemcpyArrayToArray(struct cudaArray *dst, size_t wOffsetDst,
                       size_t hOffsetDst, const struct cudaArray *src,
...
extern cudaError_t cudaMemcpy2DFromArray(void *dst, size_t dpitch,
                                         ...
                                         enum cudaMemcpyKind kind);
extern cudaError_t
cudaMemcpy2DArrayToArray(struct cudaArray *dst, size_t wOffsetDst,
                         size_t hOffsetDst, const struct cudaArray *src,
...
extern cudaError_t
cudaMemcpyFromSymbol(void *dst, const char *symbol, size_t count,
                     size_t offset = 0,
...
extern cudaError_t cudaMemcpyToArrayAsync(struct cudaArray *dst, size_t wOffset,
                                          ...
                                          cudaStream_t stream);
extern cudaError_t
cudaMemcpyFromArrayAsync(void *dst, const struct cudaArray *src, size_t wOffset,
...
extern cudaError_t cudaGetChannelDesc(struct cudaChannelFormatDesc *desc,
                                      const struct cudaArray *array);
extern struct cudaChannelFormatDesc
...
extern cudaError_t
cudaGraphicsUnregisterResource(struct cudaGraphicsResource *resource);
extern cudaError_t
cudaGraphicsResourceSetMapFlags(struct cudaGraphicsResource *resource,
                                unsigned int flags);
extern cudaError_t
cudaGraphicsMapResources(int count, struct cudaGraphicsResource **resources,
                         cudaStream_t stream = 0);
extern cudaError_t
cudaGraphicsUnmapResources(int count, struct cudaGraphicsResource **resources,
                           cudaStream_t stream = 0);
extern cudaError_t
